  ...
  chrome.storage.local.get(null, (items) => {
    ...
    if (keysToRemove.length > 0) {
      chrome.storage.local.remove(keysToRemove, () => {
        ...
        if (status)
          status.textContent = `Cleared ${keysToRemove.length} choice(s).`;
        setTimeout(() => {
          if (status)
            status.textContent = "";
        }, 2e3);
      ...
      const status = document.getElementById("cache-status");
      if (status)
        status.textContent = "No choices to clear.";
      setTimeout(() => {
        if (status)
          status.textContent = "";
      }, 2e3);
...
var pubkeyInput = document.getElementById("pubkey-input");
var pubkeyStatus = document.getElementById("pubkey-status");
  ...
  if (pubkey) {
    if (pubkey.startsWith("npub1") || /^[0-9a-fA-F]{64}$/.test(pubkey)) {
      chrome.storage.local.set({ "user_pubkey": pubkey }, () => {
        if (pubkeyStatus)
          pubkeyStatus.textContent = "Pubkey saved!";
        setTimeout(() => {
          if (pubkeyStatus)
      ...
      if (pubkeyStatus)
        pubkeyStatus.textContent = "Invalid pubkey format.";
      setTimeout(() => {
        if (pubkeyStatus)
          pubkeyStatus.textContent = "";
      }, 2e3);
